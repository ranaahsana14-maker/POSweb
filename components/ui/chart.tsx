...
// ---------------- TOOLTIP ----------------
// `TooltipPayload` was removed/renamed in newer recharts versions.  the
// exported alias is `TooltipPayloadEntry`, which is the same generic
// payload type used by the internal state slice.
type TooltipItem = RechartsPrimitive.TooltipPayloadEntry<number, string> & { payload?: Record<string, any> }

interface ChartTooltipContentProps extends Omit<RechartsPrimitive.TooltipProps<number, string>, 'content'> {
  hideLabel?: boolean
              ...
              {formatter && item.value !== undefined && item.name
                ...
                : <>
                    {itemConfig?.icon ? <itemConfig.icon /> : !hideIndicator && (
                      <div
                        className={cn('shrink-0 rounded-[2px] border-(--color-border) bg-(--color-bg)', indicator === 'dot' ? 'h-2.5 w-2.5' : '', indicator === 'line' ? 'w-1' : '', indicator === 'dashed' ? 'w-0 border-[1.5px] border-dashed bg-transparent my-0.5' : '')}
  ...
  return (
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map((item: any, idx: number) => {
        ...
        const itemConfig = getPayloadConfig(config, item, key)
        return (
          <div key={item.value || idx} className='[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3'>
...
function getPayloadConfig(config: ChartConfig, payload: any, key: string) {
  ...
  else if (p && key in p && typeof p[key] === 'string') cfgKey = p[key]
  return cfgKey in config ? config[cfgKey] : config[key]
}
